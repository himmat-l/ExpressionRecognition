...
sys.path.append("../")
from mtcnn.MTCNN_config import config
...
class MtcnnDetector(object):
    ...
    def __init__(self,
                 ...
                 min_face_size=20,
                 stride=2,
                 threshold=[0.6, 0.7, 0.7],   # 一个包含3个值的列表，这3个值在算法的3个stage中将分别用到，可以看到这3个threshold值是递增的，是因为在3个stage中对一个bbox是否是人脸的置信度要求越来越高。
                 scale_factor=0.79,   # 和图像金字塔相关的一个参数，表示图像金字塔的每相邻两层之间的倍数关系是factor
    ...
    def convert_to_square(self, bbox):
        ...
        ----------
            bbox: numpy array , shape n x 5
                n表示检测到的人脸数量，5表示每张人脸有4个坐标点（左上角的x，y和右下角的x，y）和1个置信度score
                input bbox
        Returns:
        -------
            square bbox
